...
int pwr_on_thrsd = 2;          //power on capcity threshold
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	else
	{
		...
		{
			printk("%s..with error string: %s %d %d\n",__FUNCTION__,buf,strncmp(buf,resume,3),strncmp(buf,suspend,3));
			return count;
		}
	}
}
