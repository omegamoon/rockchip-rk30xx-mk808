...
//  		}
//  		pr_debug("\n");
			copy_to_user((u8 __user *)(uintptr_t) u_tmp->rx_buf, tx_buf + (USB_LEADING_TAG_OFFSET - 1), u_tmp->len);
		}
	}
...
//  		pr_debug("\n");
			...
			__copy_to_user((u8 __user *)(uintptr_t) u_tmp->rx_buf, rx_buf, image_size);
...
int Egis_IO_Bulk_Read(usb_ss801u *dev, int arg)
{
	...
	//_____Step3. if send successfully, copy read data to user.
   	...
   	if(EGIS_SUCCESS(retval)){
		...
		if(0 != retval)
		{
			if(1024 > Bulk_buffer.DataSize) memcpy(dataBuf, Bulk_buffer.Data, Bulk_buffer.DataSize);
