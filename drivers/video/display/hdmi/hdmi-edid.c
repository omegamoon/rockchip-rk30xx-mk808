...
static int hdmi_edid_get_cea_svd(unsigned char *buf, struct hdmi_edid *pedid)
{
	...
	for(i = 0; i < count; i++)
	{
		...
		if(vic)
		{
			mode = ext_hdmi_vic_to_videomode(vic);
			if(mode)
			{	
				ext_hdmi_add_videomode(mode, &pedid->modelist);
			}
		}
	}
	return 0;
...
#if 1	
{
	...
	while(ddc_offset < HDMI_EDID_BLOCK_SIZE - 2)	//buf[126] = 0 and buf[127] = checksum
	{
		...
		ext_hdmi_add_videomode(vmode, &pedid->modelist);
		ddc_offset += 18;
	}
	kfree(vmode);
}
...
int HDMI_EDID_parse_1st_extension(unsigned char *buf, struct hdmi_edid *pedid)
{
	int rc;
    ...
    {
    	hdmi_edid_error("[EDID] 1st extensions block is not CEA type.\n");
    	return E_HDMI_EDID_CHECKSUM;
    }
    return hdmi_edid_parse_extensions_cea(buf, pedid);
}
