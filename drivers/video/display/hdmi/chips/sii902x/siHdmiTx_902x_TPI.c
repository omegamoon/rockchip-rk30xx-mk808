...
void HDCP_CheckStatus (byte InterruptStatusImage)
{
	...
	{
		#if 1 //Disabled by zhy for itv driver 20110924 
...
int sii902x_detect_device(struct sii902x_pdata *anx)
{
	return (siHdmiTx_TPI_Init() == TRUE)?1:0;
}

int sii902x_sys_init(struct hdmi *hdmi)
{
	return HDMI_ERROR_SUCESS;
//	return (siHdmiTx_TPI_Init() == TRUE)?HDMI_ERROR_SUCESS:HDMI_ERROR_FALSE;
}

int sii902x_sys_detect_hpd(struct hdmi *hdmi, int *hpdstatus)
{
...
//		printk("InterruptStatus %02x\n", InterruptStatus);
		...
		#ifdef DEV_SUPPORT_HDCP
		if (hdmi->config_set.hdcp_on == HDMI_ENABLE
			&& (g_sys.hdmiCableConnected == TRUE)
			&& (g_sys.dsRxPoweredUp == TRUE)
			&& (g_hdcp.HDCPAuthenticated == VMD_HDCP_AUTHENTICATED))
		{
	...
	*hpdstatus = (g_sys.hdmiCableConnected == TRUE)?HDMI_RECEIVER_INACTIVE:HDMI_RECEIVER_REMOVE;
	
	return HDMI_ERROR_SUCESS;
}

int sii902x_sys_insert(struct hdmi *hdmi)
{
	TPI_DEBUG_PRINT (("Cable Connected\n"));
	#ifdef DEV_SUPPORT_HDCP
	if ((g_hdcp.HDCP_TxSupports == TRUE) 
...
int sii902x_sys_remove(struct hdmi *hdmi)
{
	TPI_DEBUG_PRINT (("HDMI Disconnected\n"));
	OnDownstreamRxPoweredDown();
	return HDMI_ERROR_SUCESS;
...
int sii902x_sys_read_edid(struct hdmi *hdmi, int block, unsigned char *buff)
{
	return (siHsiHdmiTx_ReadEdid(block, buff) == EDID_OK)?HDMI_ERROR_SUCESS:HDMI_ERROR_FALSE;
}

int sii902x_sys_config_video(struct hdmi *hdmi, int vic, int input_color, int output_color)
{
	// ÅäÖÃÊÓÆµÊä³ö
	byte vmode;
	switch(vic)
	...
	siHsiHdmiTx_VideoConfig(hdmi->edid.is_hdmi);
	return HDMI_ERROR_SUCESS;
}

int sii902x_sys_config_audio(struct hdmi *hdmi, struct hdmi_audio *audio)
{
	byte MCLK_FS;
...
#define HDCP_MAX_TRY_TIME	10
int sii902x_sys_config_hdcp(struct hdmi *hdmi, int enable)
{		
	#ifdef DEV_SUPPORT_HDCP
	if ((g_sys.hdmiCableConnected == TRUE)
			&& (g_sys.dsRxPoweredUp == TRUE)
			&& (g_hdcp.HDCPAuthenticated == VMD_HDCP_AUTHENTICATED))
		{
			if(enable)
				HDCP_CheckStatus(ReadByteTPI(TPI_INTERRUPT_STATUS_REG));
			else if (g_hdcp.HDCP_Started == TRUE)
				HDCP_Off();
		}
	#endif
	return HDMI_ERROR_SUCESS;
...
int sii902x_sys_enalbe_output(struct hdmi *hdmi, int enable)
{
	if(enable == HDMI_DISPLAY_ON)
	{
		#ifdef DEV_SUPPORT_HDCP
		if ( (hdmi->config_set.hdcp_on == HDMI_ENABLE) && (g_hdcp.HDCP_TxSupports == TRUE) && (g_hdcp.HDCP_AksvValid == TRUE)) {
//			if(g_hdcp.HDCP_LinkProtectionLevel == LOCAL_LINK_PROTECTION_SECURE)
//			{
//				TPI_DEBUG_PRINT (("TMDS -> Enabled\n"));
//				ReadModifyWriteTPI(TPI_SYSTEM_CONTROL_DATA_REG, LINK_INTEGRITY_MODE_MASK | TMDS_OUTPUT_CONTROL_MASK | AV_MUTE_MASK, 
//					LINK_INTEGRITY_DYNAMIC | TMDS_OUTPUT_CONTROL_ACTIVE | AV_MUTE_NORMAL);
