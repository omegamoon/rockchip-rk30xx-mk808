...
s32 gup_enter_update_mode(struct i2c_client *client)
{
    ...
    while(retry++ < 200)
    {
        ...
        {
            GTP_INFO("Enter update Hold ss51 SUCCESS.");
            break;
        }
    }
    if(retry >= 200)
    {
        GTP_INFO("Enter update Hold ss51 failed.");
    }
    //20121114 modify end
    return ret;
}
...
static u8 gup_burn_fw_section(struct i2c_client *client, u8 *fw_section, u16 start_addr, u8 bank_cmd )
{
    ...
    ret = gup_set_ic_msg(client, _bWO_MISCTL__CPU_SWRST_PULSE, 0x01);           //20121114
