...
#if 1
#define TOUCH_MAX_HEIGHT   1024 //768 //7680
#define TOUCH_MAX_WIDTH     768 //1024//5120
#else
#define AUTO_SET
...
s32 goodix_init_panel(struct goodix_ts_data *ts, u8 send)
{
    ...
    0x0A,0x00,0x1B,0x03,0x68,0x10,0x10,0x13,
    0x00,0x00,0x0C,0x00,0x00,0x0E,0x50,0x30,
    0x11,0x03,0x00,0x05,0x00,0x10,0x00,0x10,
    0x00,0x5A,0x5A,0x46,0x46,0x04,0x00,0x03,
    0x19,0x05,0x14,0x10,0x00,0x06,0x00,0x00,
   ...
   /* 0x1D,0x0E,0x1C,0x0D,0x1B,0x0C,0x1A,0x0B,
    ...
    0x12,0x08,0x11,0x07,0x10,0x06,0x0F,0x05,
    0x0E,0x04,0x0D,0x03,0x0C,0x02,0x0B,0x01,
    0x0A,0x00,0x1B,0x03,0x48,0x10,0x10,0x16,  //to 0x1a
    0x00,0x00,0x0C,0x00,0x00,0x06,0x50,0x30,
    0x11,0x03,0x00,0x05,0x00,0x10,0x00,0x10,
    0x00,0x5A,0x5A,0x46,0x46,0x04,0x00,0x03,
    0x19,0x05,0x14,0x10,0x00,0x06,0x00,0x00,
    0x00,0x00,0x00,0x00,0x38,0x28,0x60,0x20,
    ...
    0x12,0x08,0x11,0x07,0x10,0x06,0x0F,0x05,
    0x0E,0x04,0x0D,0x03,0x0C,0x02,0x0B,0x01,
    0x0A,0x00,0x1B,0x03,0x68,0x10,0x10,0x1A,
    0x00,0x00,0x0C,0x00,0x00,0x0E,0x50,0x30,
    0x11,0x03,0x00,0x05,0x00,0x10,0x00,0x10,
    0x00,0x5A,0x5A,0x46,0x46,0x04,0x00,0x03,
    0x19,0x05,0x14,0x10,0x00,0x06,0x00,0x00,
...
/**///GTDEBUG_COOR("check sum byte:%x\n", coor_point[5*finger]);
    ...
    /**/    //    GTDEBUG_COOR("Y:%d\n", (s32)Y_value);
		//printk("Finger press dix:%d X:%04d Y:%04d\n",position, (s32)Y_value,(s32)X_value);
            }
...
/**/for (i = 0; i < 4; i++)
/**/{
/**/    GTDEBUG_COOR("key:%4x   ", !!(key&(0x01<<i)));
/**/}
/**/GTDEBUG_COOR("\n");
#endif
...
//    ts->input_dev->keybit[BIT_WORD(BTN_TOUCH)] = BIT_MASK(BTN_TOUCH);
 //   ts->input_dev->absbit[0] = BIT(ABS_X) | BIT(ABS_Y) | BIT(ABS_PRESSURE);// absolute coor (x,y)
	...
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, /*TOUCH_MAX_HEIGHT*/4096, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, /*TOUCH_MAX_WIDTH*/4096, 0, 0);
...
static s32 goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //Test I2C connection. 
    GTDEBUG_MSG("Testing I2C connection...\n");
